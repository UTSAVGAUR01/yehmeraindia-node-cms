...
function cleanContentForPublic(content) { return String(content || '').replace(/\n\n#.*$/s, ''); }
...
app.get('/api/posts', async (req, res) => {
  try {
    ...
    res.json({ success: true, source: 'database', posts: rows.map(p => ({ ...p, content: cleanContentForPublic(p.content) })) });
...
app.post('/api/posts', requireAdminApi, async (req, res) => {
  const p = req.body || {};
  const post = { id: Date.now(), title: p.title || 'Untitled Post', category: p.category || 'Vivid India', excerpt: p.excerpt || '', content: p.content || '', hashtags: p.hashtags || '', media_url: p.media_url || '', created_at: new Date().toISOString() };
  try {
...
app.put('/api/posts/:id', requireAdminApi, async (req, res) => {
  ...
  try {
    ...
    memoryPosts = memoryPosts.map(x => x.id === id ? { ...x, title: p.title, category: p.category, excerpt: p.excerpt, content: p.content, hashtags: p.hashtags, media_url: p.media_url } : x);
...
['b','c','s','sep','hue'].forEach(id=>qs(id).oninput=applyMedia);photo.onchange=e=>{let f=e.target.files[0];if(f){preview.src=URL.createObjectURL(f);applyMedia()}};
function mediaPreset(name){let values={reset:[1,1,1,0,0],vivid:[1.15,1.25,1.6,0.1,0],heritage:[1.08,1.15,1.2,0.45,8],monsoon:[1.05,1.2,1.55,0,-12],desert:[1.18,1.25,1.35,0.35,12]}[name];b.value=values[0];c.value=values[1];s.value=values[2];sep.value=values[3];hue.value=values[4];applyMedia()}
function downloadImg(){if(!preview.src)return alert('Upload image first');let a=document.createElement('a');a.href=preview.src;a.download='yehmeraindia-studio-ai-media.png';a.click()}loadPosts();
</script>`)));
