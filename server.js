...
function getDemoAiReporterPosts() {
  return [
    { title: 'AI Reporter: Fresh India Updates Every Hour', slug: 'ai-reporter-fresh-india-updates-every-hour', summary: 'Hourly AI-assisted summaries for key Indian developments.', excerpt: 'Fresh India updates with responsible AI newsroom workflow.', category: 'ai-reporter', source_name: 'Yeh Mera India AI Desk', visual: 'linear-gradient(165deg,#ff8f00,#0b1f4d)', is_breaking: 1, ai_generated: 1, ai_reporter_name: 'Asha AI', published_at: new Date() },
    { title: 'Bharat Innovation: How AI Can Help Local News', slug: 'bharat-innovation-how-ai-can-help-local-news', summary: 'Regional reporting gets faster with human-reviewed AI drafting.', excerpt: 'AI tools can support district-level reporting with verification.', category: 'india-stories', source_name: 'Innovation Desk', visual: 'linear-gradient(165deg,#138808,#0b1f4d)', is_breaking: 0, ai_generated: 1, ai_reporter_name: 'Asha AI', published_at: new Date() },
    { title: 'Culture & Heritage: India Stories in Digital Format', slug: 'culture-heritage-india-stories-in-digital-format', summary: 'Digital storytelling celebrates India\'s culture in modern news formats.', excerpt: 'Culture-led explainers and reels for younger audiences.', category: 'india-stories', source_name: 'Culture Desk', visual: 'linear-gradient(165deg,#ff8f00,#138808)', is_breaking: 0, ai_generated: 1, ai_reporter_name: 'Asha AI', published_at: new Date() },
    { title: 'Government Schemes: What Citizens Should Verify Before Applying', slug: 'government-schemes-what-citizens-should-verify-before-applying', summary: 'Checklist for citizens to validate eligibility and official links.', excerpt: 'Verify official portals and eligibility before sharing personal data.', category: 'government-schemes', source_name: 'Public Service Desk', visual: 'linear-gradient(165deg,#0b1f4d,#ff8f00)', is_breaking: 1, ai_generated: 1, ai_reporter_name: 'Asha AI', published_at: new Date() }
  ];
}
...
app.get('/api/ai-feed', (req, res) => {
  db.query(
    ...
    `,
    (err, results) => {
      res.json(err || !results?.length ? getDemoAiReporterPosts() : results);
    }
  );
});
...
app.get('/api/posts', (req, res) => {
  db.query(
    ...
    `,
    (err, results) => {
      if (err || !results?.length) return res.json(getDemoAiReporterPosts());
      res.json(results);
    }
  );
});
...
app.get('/api/admin/dashboard', requireRole(['admin']), async (req, res) => {
  ...
  res.json({ totalPosts: Number(posts.totalPosts||0), publishedPosts: Number(posts.publishedPosts||0), pendingPosts: Number(posts.pendingPosts||0), aiGeneratedPosts: Number(posts.aiGeneratedPosts||0), trendingPosts: Number(posts.trendingPosts||0), users: Number(users.users||0), deletedToday: Number(deleted.deletedToday||0), lastCleanupAt: last ? last.created_at : null, recentPosts: recentPosts || [], pendingAiPosts: pendingAiPosts || [] });
});
...
app.post('/api/admin/seed-demo-posts', requireRole(['admin']), async (req,res)=>{
  const demos = getDemoAiReporterPosts();
  for (const p of demos.concat(demos.slice(0,2))) {
    const slug = `${p.slug}-${Math.floor(Math.random()*10000)}`;
    await runQuery(`INSERT INTO posts (title,slug,excerpt,summary,content,category,status,ai_generated,is_trending,is_breaking,source_name,published_at,expires_at,is_protected) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,DATE_ADD(NOW(), INTERVAL 24 HOUR),0)`, [p.title,slug,p.excerpt,p.summary,`${p.summary}

${p.excerpt}` ,p.category,'published',1,p.is_breaking?1:0,p.is_breaking?1:0,p.source_name,new Date()]);
  }
  res.json({message:'Demo AI news posts created', count:6});
});
